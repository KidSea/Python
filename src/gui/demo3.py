...
def getMove(weight, density, p, c, r, A):
    g = 9.8
    top = 2 * weight * g * (density - p)
    bottom = c * density * p
    temp = abs(top / bottom)
    v = math.sqrt(temp)
    F = 0.5 * c * A * p * v
...
class Application(Tk):
    ...
    def caculate(self):
        ...
        p, c , A= self.getAirParams(self.air_list_box.get())
        r = self.getRadius(self.caliber_list_box.get())
        D = getMove(weight, density, p, c, r, A)
        print("value", weight, density, p, c, r, A, D)
        self.result_text.insert(INSERT,'D = ' + str(D) + '\n')
    ...
    def getAirParams(self, param):
        if param == '空气':
            return 1.29, 3.02, 2
        if param == '氢气':
            return 0.0899, 2.17, 1
        if param == '氧气':
            return 1.429, 3.52, 2.5
        if param == '氮气':
            return 1.25, 2.99, 2
